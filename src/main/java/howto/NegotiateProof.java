...
import static org.hyperledger.indy.sdk.ledger.Ledger.parseGetCredDefResponse;
...
//        });
        ...
        System.out.println("credential.getCredentialJson() : " + credential.getCredentialJson());
        System.out.println("credential.getRevocId() : " + credential.getRevocId());
        System.out.println("credential.getRevocRegDeltaJson() : " + credential.getRevocRegDeltaJson());
        ...
        String revocStates = new JSONObject().toString(); //폐기여부를 검증해야 하는 VC일 경우 블록체인에서 가져와야 함.
        ...
        LedgerResults.ParseResponseResult schemaFromLedgerForVerifier = IndyUtil.getSchema(pool, govermentDid, receivedSchemaId);
        LedgerResults.ParseResponseResult credDefFromLedgerForVerifier = IndyUtil.getCredDef(pool, govermentDid, receivedCredDefId);
        ...
        Boolean valid = verifierVerifyProof(proofRequestJson, proofJson, schemasForVerifier, credentialDefsForVerifier, revocRegDefs, revocRegs).get();
        assertTrue(valid);
        ...
        System.out.println("\n################## howto.NegotiateProof -> completed ##################\n");
    }
